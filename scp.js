...
// --- Durum Yönetimi ---
let isLoggedIn = false; // 🔥 YENİ DURUM: Giriş Yapıldı mı?
let currentMode = 'LOGIN'; // 🔥 BAŞLANGIÇ MODU: LOGIN olarak ayarlandı
let isDrawing = false;
let color = '#000000';
let lineThickness = 5;
...
// 🔥 YENİ DOM: Login mesajı ve giriş görünümü
...
/**
 * 🔥 YENİ: Analiz Başlatma Düğmesi Tıklaması
 */
window.handleStartAnalysis = function () {
    if (isLoggedIn) {
        render('DRAWING');
    } else {
        // Eğer login değilse, login ekranına yönlendir ve uyarı göster
        render('LOGIN');
        if (loginMessage) {
            loginMessage.textContent = "⛔ Analize başlamak için lütfen önce giriş yapın.";
            loginMessage.classList.remove('hidden');
        }
    }
}
...
window.analyzeDrawing = async function () {
    if (!canvas) return;
    if (!isLoggedIn) {
        window.handleStartAnalysis(); // Login kontrolünü tekrar çağır
        return;
    }
...
window.resetApp = function () {
    ...
    // Sayfayı sıfırla, giriş durumuna göre HOME veya LOGIN'e dön
    if (isLoggedIn) {
        render('HOME');
    } else {
        render('LOGIN');
