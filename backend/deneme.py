...
COLOR_PSYCHOLOGY_TR = {
    # HEX kodlarına göre güncellendi
...
ANALYSIS_PROFILES = [
    ...
    # Skor 10-15: Düşük Yoğunluk - Hafif
    ...
    # Skor 15-20: Düşük Yoğunluk - Duyarlı
    ...
    # Skor 20-25: Düşük-Orta Yoğunluk - Dengeli
    ...
    # Skor 25-30: Orta-Düşük Yoğunluk - Meraklı
    ...
    # Skor 30-35: Orta-Düşük Yoğunluk - Odaklanmış
    ...
    # Skor 35-40: Orta Yoğunluk - Yaratıcı
    ...
    # Skor 40-45: Orta Yoğunluk - Coşkulu
    ...
    # Skor 45-50: Orta-Yüksek Yoğunluk - Tutkulu
    ...
    # Skor 50-55: Orta-Yüksek Yoğunluk - Duygusal
    ...
    # Skor 55-60: Yüksek Yoğunluk - Enerjik
    ...
    # Skor 60-65: Yüksek Yoğunluk - Dirençli
    ...
    # Skor 65-70: Yüksek Yoğunluk - Gergin
    ...
    # Skor 70-75: Çok Yüksek Yoğunluk - Çatışmalı
    ...
    # Skor 75-80: Çok Yüksek Yoğunluk - Baskı Altında
    ...
    # Skor 80-85: Kritik Yoğunluk - Tükenmiş
    ...
    # Skor 85-90: Kritik Yoğunluk - Bunalmış
    ...
    # Skor 90-95: Aşırı Yoğunluk - Kaotik
    ...
    # Skor 95-100: Maksimum Yoğunluk - Krizi
...
def analyze_canvas_usage(image_array):
    ...
    # Sayfayı 5 eşit bölgeye ayır (merkez bölge dahil)
    h_third = height // 3
    w_third = width // 3
...
def analyze_line_thickness(image_array):
    ...
    # Gri tonlamaya çevir (float for gradient)
    gray = np.mean(image_array, axis=2).astype(np.float32)
    
    # NumPy tabanlı yaklaşık kenar tespiti: görüntüdeki yatay/dikey gradyanları hesapla
    gy, gx = np.gradient(gray)
    edges = np.hypot(gx, gy)
    
    # Kenar yoğunluğuna göre kalınlık tahmini (eşik, ortalamaya dayalı)
    if edges.size == 0:
        edge_density = 0
    else:
        thresh = max(1e-6, edges.mean() * 0.5)
        edge_density = np.mean(edges > thresh) * 1000  # Ölçeklendirilmiş yoğunluk
...
def generate_region_analysis(region_density):
    ...
    second_region = sorted_regions[1][0] if len(sorted_regions) > 1 and sorted_regions[1][1] > max_density * 0.7 else None
    
    analysis = f"En yoğun kullanım {max_region.replace('_', ' ')} bölgesinde (%{max_density:.1f}). {region_meanings[max_region]}"
    
    if second_region and second_region != max_region:
        analysis += f" İkincil odak {second_region.replace('_', ' ')} bölgesinde, bu da {region_meanings[second_region].split('.')[0].lower()} eğilimini destekliyor."
...
def generate_color_modifier(top_color_name, psychology_text):
    """Baskın renge dayalı psikolojik rapor cümlesi oluşturur."""
    color_modifiers = {
        "Siyah": "Baskın Siyah kullanımı, **güçlü içsel düşünceleri** ve derin duygusal yoğunluğu gösteriyor.",
        "Kırmızı": "Kırmızı hakimiyeti, **tutkulu enerji** ve güçlü duygusal tepkilerin varlığına işaret ediyor.",
        "Mavi": "Baskın Mavi tonları, **sakin bir zihin** ve içsel huzur arayışını yansıtıyor.",
        "Yeşil": "Yeşil kullanımı, **denge ihtiyacı** ve doğal büyüme arzusunu gösteriyor.",
        "Altın Sarısı": "Altın Sarısı tonları, **yaratıcı bilgelik** ve entelektüel merakın güçlü olduğunu belirtiyor.",
        "Mor": "Mor hakimiyeti, **derin yaratıcılık** ve ruhsal arayışları ön plana çıkarıyor.",
        "Turuncu": "Turuncu kullanımı, **sosyal enerji** ve coşkulu iletişim isteğini gösteriyor.",
        "Kahverengi": "Kahverengi tonları, **pratik yaklaşım** ve güvenlik arayışını yansıtıyor.",
        "Pembe": "Pembe hakimiyeti, **duygusal hassasiyet** ve sevgi arayışının güçlü olduğunu belirtiyor.",
        "Gri": "Gri kullanımı, **duygusal mesafe** ve nötr bir yaklaşımı gösteriyor."
    }
    
    return color_modifiers.get(top_color_name, f"Baskın {top_color_name} kullanımı, {psychology_text.split('.')[0].lower()} durumunu güçlendiriyor.")
...
def analyze_drawing_density(image_base64: str) -> dict:
    ...
    # Çizilmiş pikselleri saymak için eşik
    ...
    thickness_influence = thickness_score * 2  # Kalınlık skorunun etkisi

    # GELİŞMİŞ KOMPOZİT DUYGUSAL YOĞUNLUK SKORU (0-100)
    ...
    # --- GELİŞMİŞ RAPOR OLUŞTURMA ---
    emotion = selected_profile["emotion"]
    psychology_report_parts = [selected_profile["base_psychology"]]
    ...
    if psychological_color_counts:
        ...
        # 1. Baskın Renk Modifikatörü
        psychology_report_parts.append(generate_color_modifier(top_color_name, top_color_psychology))

        # 2. Renk Çeşitliliği Modifikatörü
        if unique_color_count > 40:
             psychology_report_parts.append(f"Çok yüksek renk çeşitliliği ({unique_color_count} ton), zihninizin **karmaşık ve detay odaklı bir yelpazede** çalıştığını, potansiyel olarak **aşırı uyarılmış** olduğunu gösterir.")
        elif unique_color_count < 5:
             psychology_report_parts.append(f"Sadece {unique_color_count} renk tonu kullanılması, duygusal ifadenin **kısıtlı bir alana** odaklandığını veya **içe dönük bir sınırlama** yaşandığını düşündürür.")
        else:
             psychology_report_parts.append(f"Sağlıklı renk çeşitliliği, duygusal yelpazenizin **geniş ve uyumlu** olduğunu göstermektedir.")

        # 3. Koyu Ton Modifikatörü
        if darkness_score > 60:
            psychology_report_parts.append("Yüksek Koyu Renk Yoğunluğu (Darkness Score), algılanan durumun **ağırlığını ve ciddiyetini** artırarak, **derin bir duygusal baskıya** işaret etmektedir.")
        elif darkness_score < 30:
            psychology_report_parts.append("Düşük Koyu Renk Yoğunluğu, yaklaşımınızın **hafif, optimist ve savunmasız** olduğunu gösterir.")
        
    else:
        psychology_report_parts.append("Renk verisi yetersiz olduğu için derin psikolojik renk analizi yapılamamıştır.")

    # 4. SAYFA BÖLGESİ ANALİZİ (5 BÖLGE)
    region_analysis = generate_region_analysis(region_density)
    psychology_report_parts.append(region_analysis)
    
    # 5. KALEM KALINLIĞI ANALİZİ
    thickness_analysis = generate_thickness_analysis(thickness_level, thickness_score)
    psychology_report_parts.append(f"Kalem kalınlığı ({thickness_level}): {thickness_analysis}")

    # --- Sonuçları Derleme ---
    psychology = " ".join(psychology_report_parts)
    
    # Gelişmiş Color Analysis
