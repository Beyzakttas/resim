...
function initApp() {
    ...
    // Modal Dışına Tıklama Dinleyicisi
    if (alertModal) {
...
window.handleLogin = function (event) {
    ...
    if (username && password) {
        ...
        // Başarılı giriş sonrası HOME ekranına yönlendir, uyarıya gerek yok.
        render('HOME');
    } else {
        if (loginMessage) {
...
window.handleStartAnalysis = function () {
    if (isLoggedIn) {
        ...
        // İstenen uyarı metni formatı: Giriş gerekli
        showAlert("Giriş Gerekli", "Çizim analizine başlamak için lütfen önce **Giriş Yapın**.");
    }
}

/**
 * Oturumu Kapatma / Giriş Yapma Butonu İşlevi
 */
window.resetApp = function () {
    ...
    if (isLoggedIn) {
        // Oturum kapatma ve uyarı modalı göster
        isLoggedIn = false;
        showAlert("Oturum Kapatıldı", "Başarıyla oturumunuzu kapattınız."); // Çıkış uyarısı
        render('HOME');
    } else {
        // Giriş yapma ekranına geçiş
        render('LOGIN');
    }
}
...
function showAlert(title, message) {
    ...
    const titleEl = alertModal.querySelector('strong'); // Başlık etiketi
...
function render(mode) {
    ...
    // Main konteyner düzenini ayarla
    document.getElementById('mainContainer').classList.remove('items-start');
    document.getElementById('mainContainer').classList.add('items-center');
    ...
    switch (mode) {
        ...
        case 'DRAWING':
            ...
            document.getElementById('mainContainer').classList.remove('items-center');
            document.getElementById('mainContainer').classList.add('items-start');
...
function updateUI() {
    // Chat butonlarının ve inputunun durumu
    if (chatInput && chatSendButton) {
        chatInput.disabled = isChatLoading;
        chatSendButton.disabled = isChatLoading || chatInput.value.trim() === '';
    }

    // Ana Sayfa Giriş Butonu Metni (İkonlar kaldırıldı)
    const loginToggle = document.getElementById('loginToggle');
    if (loginToggle) {
        loginToggle.textContent = isLoggedIn
            ? 'Oturumu Kapat'
    ...
    const updateButtonState = (button, isDisabled) => {
        ...
        const activeClasses = ['bg-green-100', 'text-green-700', 'hover:bg-green-200', 'cursor-pointer', 'text-opacity-100'];
        const disabledClasses = ['bg-gray-200', 'text-gray-400', 'cursor-not-allowed', 'text-opacity-50', 'hover:bg-gray-200'];
...
function initCanvas(forceClear = false) {
    ...
    // Maksimum boyutları ve responsive oranı ayarla
...
function renderChatHistory() {
    ...
    if (chatHistory.length === 0 && !isChatLoading) {
        // Boş mesaj konteyneri bırakıldı, ilk mesaj gönderme teşviği
    } else {
...
function updateToolsUI() {
    ...
    // Renk butonları
    const colors = ['#000000', '#FF0000', '#0000FF', '#32C832', '#FFD700', '#9400D3', '#ffa54f', '#8b4513', '#ff34b3', '#8b8b7a'];
    const colorButtons = colors.map(c => `
        <button 
            onclick="setColorTool('${c}')"
            class="w-8 h-8 rounded-full border-4 transition-all duration-150 ${color === c && tool === 'pen' ? 'border-indigo-600 shadow-md ring-2 ring-indigo-300' : 'border-gray-200 hover:border-gray-400'}"
            title="${c}"
    ...
    drawingToolsDiv.innerHTML = `
        ...
        >
            <i data-lucide="pen-tool" class="mr-1"></i> Kalem
        </button>
        <button
        ...
        >
            <i data-lucide="eraser" class="mr-1"></i> Silgi
        </button>
        ...
        <div class="flex space-x-1.5 ml-2 items-center">
            ${colorButtons}
        </div>
        ...
        <button 
            ...
            class="p-2 rounded-lg shadow-sm transition flex items-center bg-red-100 text-red-700 hover:bg-red-200 font-medium"
            title="Tüm tuvali beyaz ile temizle"
        >
            <i data-lucide="trash-2" class="mr-1"></i> Temizle
        </button>
        ...
        >
            <i data-lucide="brain" class="mr-1"></i> Analiz Et
        </button>
    `;
